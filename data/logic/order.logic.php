...
class orderLogic {
    ...
    public function changeOrderReceivePay($order_list, $role, $user = '', $post = array()) {
        ...
        foreach($order_list as $order_info) {
			//防止重复发送消息 v 5 . 2 - 3 3 h a o . c o m
	...
	public function addInviteRate($order_info){
			...
			//取得拥金金额
			 ...
			 if($commis_amount_sum>0) {
				  ...
				  $invite_money2 = ceil($commis_amount_sum * $GLOBALS['setting_config']['hao_invite2']*0.01);
				  $invite_money3 = ceil($commis_amount_sum * $GLOBALS['setting_config']['hao_invite3']*0.01);
			 }
			//检测是否货到付款方式
