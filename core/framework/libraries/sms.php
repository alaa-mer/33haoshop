...
class Sms
{
    ...
    private function mysend_verifycode_dayu( $mobile, $content)
    {
        $sms_config = config_item("sms");
        if(isset($sms_config['dayu'])){
            require_once(BASE_DATA_PATH . DS . 'api/smsdayu/SmsUtil.php');
            return SmsUtil::send($sms_config['dayu']['key'], $sms_config['dayu']['secret'],$sms_config['dayu']['signName'],$mobile,$content,$sms_config['dayu']['verify_code_tpl']);
        }else{
            return false;
        }
    }
